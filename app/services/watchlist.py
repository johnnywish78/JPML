...
from app.library.watchlist_repository import WatchlistEntry, WatchlistRepository


class WatchlistService:
    """UI-facing API for the watchlist (same semantics as favorites)."""
    ...
    def is_in_watchlist(self, entity_type: str, entity_id: int) -> bool:
        return self._repo.is_in_watchlist(entity_type, entity_id)

    def list(self, entity_type: str | None = None) -> list[WatchlistEntry]:
        return self._repo.list(entity_type)

    def get(self, entity_type: str, entity_id: int) -> WatchlistEntry | None:
    ...
    def _ensure_exists(self, entity_type: str, entity_id: int) -> None:
        ...
        if row is None:
            raise LookupError(f"{entity_type} not found: id={entity_id}")
